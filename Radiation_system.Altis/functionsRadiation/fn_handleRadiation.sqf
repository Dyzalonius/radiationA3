...
fn_handleRadiation = {
    ...
    if (_this < 0) then {
        ...
        if (missionNameSpace getVariable "radiationAceDamage") then {
            ...
            player setDamage ((damage player) + _damagePerTick);
        };
...
fn_getRadiationStrength = {
    ...
    // If player in any protection area, set radiation strength
    {
        if (((position player) distance (_x select 0)) < (_x select 1) && _radiationStrength < (_x select 2)) then {
            _radiationStrength = (_x select 2);
        };
